...
/* FSLT $f1, $f2, $f3 */
#define HANDLE_FSLT();	DBG_RUN0("Optimized HANDLE_FSLT\n");if(Rf[RAM[R[$IP]+2]] < Rf[RAM[R[$IP]+3]]){ Rf[RAM[R[$IP]+1]] = 1.0; }else{ Rf[RAM[R[$IP]+1]] = 0.0; }R[$IP]=R[$IP]+4;