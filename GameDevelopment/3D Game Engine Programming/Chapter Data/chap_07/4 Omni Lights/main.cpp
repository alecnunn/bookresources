...
HRESULT ProgramTick(void) {
   ...
   g_vcL[0].Set( sinf( fT*2.0f) + 2.0f,
                 cosf( fT*2.5f),
                 sinf( fT*2.0f) );

   g_vcL[1].Set( cosf( fT*2.5f) - 4.0f,
                 sinf( fT*1.8f) * 2.0f,
                 sinf( fT*2.3f) + 2.0f);
   ...
   g_clrL[1].fR = 0.5f + 0.5f * cosf(fT*2.0f);
   g_clrL[1].fG = 0.5f + 0.5f * sinf(fT*1.8f);
   g_clrL[1].fB = 0.5f + 0.5f * sinf(fT*2.0f);

   // clear buffers and start scene
   g_pDevice->BeginRendering(true,true,true);

   RenderLight(g_vcL[0].x, g_vcL[0].y, g_vcL[0].z);
   RenderLight(g_vcL[1].x, g_vcL[1].y, g_vcL[1].z);
   Render(-1);  // ambient
   Render(0);   // light 0
   Render(1);   // light 1
...
HRESULT Render(int n) {
   ...
   HRESULT hr = g_pDevice->GetVertexManager()->Render(g_sRoom);
...
HRESULT BuildAndSetShader(void) {
   ...
   g_pDevice->CreateVShader("omni.vsh", 0,
                            true, false, 
                            &g_Omni[0]);

   g_pDevice->CreatePShader("omni.psh", 0,
                            true, false, 
                            &g_Omni[1]);
...
/**
 * Calculate matrix for a given omni light position and its radius.
 */
ZFXMatrix CalcTransAttenNoRot(const ZFXVector &vcPos,
                              float fRadius) {
...
HRESULT BuildGeometry(void) {
   ...
   g_pDevice->GetSkinManager()->AddTexture(s, "detailmap.bmp", false, 0, NULL, 0);
