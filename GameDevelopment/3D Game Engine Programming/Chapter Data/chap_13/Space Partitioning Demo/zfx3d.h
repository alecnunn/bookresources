...
class ZFXBspTree {
   ...
   private:
      ...
      void SetRelationships(ZFXBspTree*, ZFXBspTree*);
      void CalcBoundingBox(const ZFXPolygon*, unsigned int);
...
class ZFXOctree {
   ...
   private:
      ...
      ZFXPolygon   *m_pPolys;      // if leaf node
      unsigned int  m_NumPolys;    // if leaf
      ZFXOctree    *m_pChild[8];   // eight child objects
      ZFXOctree    *m_pRoot;       // root node
      ZFXOctree    *m_pParent;     // root node
      int           m_Pos;         // NE, NW, ...

      void CalcBoundingBox(const ZFXPolygon*, unsigned int);
      void InitChildObject(int ChildID, ZFXOctree *pParent);
      void ChopListToMe(ZFXPolygon*, unsigned int);
      ...
      void SetBoundingBox(const ZFXAabb &Aabb) 
