...
// set colors if item is selected
	...
	{
		std::map<unsigned long, COLORREF>::iterator itrHighlight;
		itrHighlight = m_mapHighlights.find(nItem);
		if (itrHighlight != m_mapHighlights.end())
		{
			clrTextSave = pDC->SetTextColor((*itrHighlight).second);
			clrBkSave = m_clrBkgnd;
		}
		else
		{
