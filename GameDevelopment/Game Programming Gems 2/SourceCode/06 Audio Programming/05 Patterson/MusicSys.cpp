...
void NoteOn_Function( MusicSequencerSystem_t *pMSS, Track_t *pTrack )
{
	...
	if( pVoice )
	{
		...
		//CSynFixedPoint amplitude = CSynConvert_VelocityToAmplitude( ConvertIntegerToFixed( nVelocity ) );

		TextBoxes_Output_Printf( OUTPUTMODE_AMPLITUDES, "Vel = %3d Vol = %3d Amp = 0x%08x tick = %d\n",
			nVelocity, pTrack->m_State.m_Volume, pMSS->ms_anAudioTaper[nVelocity], pMSS->m_nWorkTick );

		//result = CSynVoice_NoteOn( pVoice->m_SynVoice, pMSS->m_nWorkTick, frequency, amplitude );
		result = CSynVoice_NoteOn( pVoice->m_SynVoice, pMSS->m_nWorkTick, frequency, pMSS->ms_anAudioTaper[nVelocity] );
...
bool MusicSequencerSystem_t::PlaySequence( int nID )
{
