...
Just run the InterSeq.exe and it will load the AIFF files and MIDI file needed as it starts.  As long as the demo runs close to 30 FPS on your machine, everything should sound good.  Press the Spacebar to start and stop the tune.  The tune starts up in state '1'.  Press the '1','2','3','4', or '5' keys to set new target states.  The '1' state is the light and gentle tune, the '5' state is the dark and heavy tune.  The '2','3', and '4' keys are states in between.  A game could set target states like this depending on the action involved.  Press the 'S' key to toggle the output display enable.  Press the 'M' key to cycle through various output display modes.  Press the Escape key to exit the demo.

In the code, the MusicSequencerSystem_t object is your man.  The public interface provided in this class is all that the code in InterSeq.cpp needs to run the audio.  Start from there and then delve deeper to get to the nitty gritty.
