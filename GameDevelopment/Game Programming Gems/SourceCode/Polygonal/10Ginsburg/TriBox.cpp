...
static
int CheckPoint(vector3 p1, vector3 p2, float alpha, long mask)
{
	vector3 plane_point;
...
//
static
int CheckLine(vector3 p1, vector3 p2, int outcode_diff)
{
...
static
int PointTriangleIntersection(vector3 p, TRI t)
{
...
int TriBoxIntersect(BBOX BBox, TRI Tri)
{
	...
	//	a voxel
	//
	Trans.x= (BBox.Max.x + BBox.Min.x) / 2;
	Trans.y= (BBox.Max.y + BBox.Min.y) / 2;
	Trans.z= (BBox.Max.z + BBox.Min.z) / 2;

	TransMax= BBox.Max - Trans;
