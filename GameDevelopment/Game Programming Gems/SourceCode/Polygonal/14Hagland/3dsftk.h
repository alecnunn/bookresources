...
typedef struct 
{
   float3ds x, z, y;			//swapped around by torgeir
} point3ds;
...
//Torgeir dicovered that this MOTHERFUCKER should be dynamic, not static, need correct size
/* kfmorphkey3ds - Object morph key */
typedef struct {
   char3ds name[11]; /* name of target morph object */
} kfmorphkey3ds;
...
typedef struct xdata3ds
{
	...
	union
	{
		...
		struct
		{								// XDATA_ENTRY with no recognizable sub chunks
			long3ds size;
			void3ds *data;
		} rawentry;
	} data;
...
void GetAmbientXData(database3ds *db, char3ds *appname, xdataraw3ds **data);
void GetMapDrawerList3ds(database3ds *db, namelist3ds **list);
#ifdef __cplusplus
}
#endif
