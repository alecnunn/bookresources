...
int main (void)
{
   ...
   {
		Flocks[0]->AddTo(Boids[0+count]);      
   }

   for( count=0; count<120; count++ )
   {
		Flocks[1]->AddTo(Boids[50+count]);
   }

   for( count=0; count<20; count++ )
   {
		Flocks[2]->AddTo(Boids[170+count]);
   }

   for( count=0; count<10; count++ )
   {
		Flocks[3]->AddTo(Boids[190+count]);
   }
/*
   ...
   Flocks[1]->AddTo(Boids[6]);
   Flocks[1]->AddTo(Boids[7]);
   Flocks[1]->AddTo(Boids[8]);
   Flocks[1]->AddTo(Boids[9]);

   Flocks[2]->AddTo(Boids[10]);
   Flocks[2]->AddTo(Boids[11]);
   Flocks[2]->AddTo(Boids[12]);
   Flocks[2]->AddTo(Boids[13]);
   Flocks[2]->AddTo(Boids[14]);
   Flocks[2]->AddTo(Boids[15]);
   Flocks[2]->AddTo(Boids[16]);

   Flocks[3]->AddTo(Boids[17]);
   Flocks[3]->AddTo(Boids[18]);

   Flocks[4]->AddTo(Boids[19]);
*/
#ifdef FLOCK_DEBUG
   for (i = 0; i < MaxFlocks; i++) {
...
void IdleFunc(void) 
{
   ...
   ProfileBegin( "Flocking Update" );
   for (i = 0; i < CFlock::FlockCount; i++) {
      CFlock::ListOfFlocks[i]->Update(deltaTime);
   }
   ProfileEnd( "Flocking Update" );
