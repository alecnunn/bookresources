...
//CHostApplication.
static CTransformApplication *g_pTransformApplication = NULL; 
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	...
	g_pTransformApplication = new CTransformApplication();

	//Run until the application returns from Go!
    g_pTransformApplication->Go();

	//Clean up the application
	delete (g_pTransformApplication);
							  ...
							  WPARAM wParam, LPARAM lParam)
{
	...
	return g_pTransformApplication->MessageHandler(hWnd, Message, 
											       wParam, lParam);
}
