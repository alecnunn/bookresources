...
//CHostApplication.
static CLightingApplication *g_pLightingApplication = NULL; 
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	...
	g_pLightingApplication = new CLightingApplication();

	//Run until the application returns from Go!
    g_pLightingApplication->Go();

	//Clean up the application
	delete (g_pLightingApplication);
							  ...
							  WPARAM wParam, LPARAM lParam)
{
	...
	return g_pLightingApplication->MessageHandler(hWnd, Message, 
											      wParam, lParam);
}
