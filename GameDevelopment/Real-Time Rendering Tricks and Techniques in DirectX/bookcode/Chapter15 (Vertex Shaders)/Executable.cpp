...
//CHostApplication.
static CBasicVertexShaderApplication *g_pShaderApplication = NULL; 
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	...
	g_pShaderApplication = new CBasicVertexShaderApplication();

	//Run until the application returns from Go!
    g_pShaderApplication->Go();

	//Clean up the application
	delete (g_pShaderApplication);
							  ...
							  WPARAM wParam, LPARAM lParam)
{
	...
	return g_pShaderApplication->MessageHandler(hWnd, Message, 
											    wParam, lParam);
}
