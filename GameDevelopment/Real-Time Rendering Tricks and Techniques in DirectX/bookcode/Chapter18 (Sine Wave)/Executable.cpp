...
//CHostApplication.
static CTechniqueApplication *g_pTechniqueApplication = NULL; 
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	...
	g_pTechniqueApplication = new CTechniqueApplication();

	//Run until the application returns from Go!
    g_pTechniqueApplication->Go();

	//Clean up the application
	delete (g_pTechniqueApplication);
							  ...
							  WPARAM wParam, LPARAM lParam)
{
	...
	return g_pTechniqueApplication->MessageHandler(hWnd, Message, 
											       wParam, lParam);
}
