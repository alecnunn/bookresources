...
{
	D3DXVECTOR4 EyePos(0.0, 10.0f, -30.0f, 0.0f);
	...
	//The first value is the near plane - it will be subtracted from 
	//all values. The second value is a scaling factor. The last two
	//are just helpful 0 and 1 constants
	D3DXVECTOR4 DepthScalers(1.0f, 0.025, 0.0f, 1.0f);

	//Set the constants...
	m_pD3DDevice->SetVertexShaderConstant(4, &DepthScalers, 1);

	//Set the current shader and draw the model
	m_pD3DDevice->SetVertexShader(m_DepthShader);
...
{
	//Delete the shaders
	m_pD3DDevice->DeleteVertexShader(m_DepthShader);
