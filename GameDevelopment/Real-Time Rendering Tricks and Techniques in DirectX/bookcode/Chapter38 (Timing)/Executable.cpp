...
//CHostApplication.
static CBasicTimingApplication *g_pTimingApplication = NULL; 
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	...
	g_pTimingApplication = new CBasicTimingApplication();

	//Run until the application returns from Go!
    g_pTimingApplication->Go();

	//Clean up the application
	delete (g_pTimingApplication);
							  ...
							  WPARAM wParam, LPARAM lParam)
{
	...
	return g_pTimingApplication->MessageHandler(hWnd, Message, 
											  wParam, lParam);
}
