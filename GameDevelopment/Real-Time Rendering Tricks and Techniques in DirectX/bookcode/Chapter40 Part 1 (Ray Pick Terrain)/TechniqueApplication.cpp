...
void CTechniqueApplication::Render()
{
	...
	D3DXVECTOR4 EyePos(7.0, 7.0f, 00.0f, 0.0f);
	...
	float CubeZPos = 3.0f * sin((float)GetTickCount() / 1000.0f);
	D3DXMATRIX Rotation;
	D3DXMATRIX CubeMatrix;
	D3DXMatrixTranslation(&CubeMatrix, 
		                  0.0f, GetHeightAt(0.0f, CubeZPos, &Rotation), 
						  CubeZPos);

	CubeMatrix = Rotation * CubeMatrix;
	...
	D3DXMatrixInverse(&CubeMatrix, NULL, &CubeMatrix);
	D3DXVec4Transform(&LightDir, &LightDir, &CubeMatrix);
	D3DXVec4Normalize(&LightDir, &LightDir);
	m_pD3DDevice->SetVertexShaderConstant(4, &LightDir, 1);
...
HRESULT CTechniqueApplication::LoadMeshes()
{
	...
	if(FAILED(D3DXLoadMeshFromX("..\\media\\SimpleTerrain.x",
								D3DXMESH_MANAGED, 
										 ...
										 D3DXMATRIX* pRotation)
{
	...
	if (Intersection)
	{
		...
		//Set the matrix
		D3DXMatrixRotationAxis(pRotation, &Axis, Angle);
