...
#define TEXSIZE 128
unsigned int Reflection;		// reflection texture
...
double aspect,sx=0.154,sy=0.154,sz=0.154,anglex=0,angley=0,anglez=0;
double tr_x=0,tr_y=0,tr_z=0;			
...
extern SoftBody TheSoftBody;
extern SoftBody TheSoftBody2;
...
void CreateTexture(void)
{
  ...
  /* Reflect in the XY plane and render the cylinder */
  glPushMatrix();
  glScalef(1.0, 1.0, -1.0);

    glBindTexture(GL_TEXTURE_2D,texture2);
	TheSoftBody.DisplayGL(2);
	TheSoftBody.DisplayNormals();
...
//	cloth3.DisplayGL(3);	
   ...
   // glEnable(GL_NORMALIZE);
	...
	TheSoftBody.CalculateNormals();
	glBindTexture(GL_TEXTURE_2D,texture2);
	TheSoftBody.DisplayGL(SoftBodydrawmode);

//	TheSoftBody.DrawTriangles();

	TheSoftBody2.CalculateNormals();
	glBindTexture(GL_TEXTURE_2D,texture2);
	TheSoftBody2.DisplayGL(SoftBodydrawmode);
	...
	glDisable(GL_TEXTURE_2D);
	glDisable(GL_BLEND);
//	TheSoftBody2.DisplayNormals();
	TheSoftBody2.DisplayNormals();
	TheSoftBody2.DisplayVelocities();
	RysujVector3(Vector3(0.01,0.01,0.01),Graw,0.01);
