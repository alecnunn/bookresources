...
matrix<float> JPIKSolver::Solve()
{
	...
	while( true )
	{
		...
		// Added just to watch progress. 
		// Eliminate to avoid high performance penalty.
		cout << "Iteration " << n << ":" << endl;
		cout << "Angles :" << endl;
		cout << Sol << endl;
		cout << "Last node position:" << endl;
		cout << fnValues( Sol, m_pClient ) << endl;

		Jacob = fnJacobian( Sol, m_pClient );

		// Jacobian pseudoinverse.
			
		PsI = (~Jacob) * !( Jacob * (~Jacob) );
