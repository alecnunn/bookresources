...
bool DemoApp::OnCreate( void ) {
	...
	GLExtensions::SetupMultitexturing( );
	...
	m_ROAM.LoadDataSet( "..\\Data\\ps_height_4k.raw", 4097 );
	m_ROAM.LoadTexture( "..\\Data\\ps_texture_1k.tga" );
	m_ROAM.Init( 23, 65536, &m_camera );
	m_ROAM.SetScale( 1.0f, 2.0f, 1.0f );
	m_ROAM.SetMaxTris( 30000 );
...
void DemoApp::ResizeScene( void ) {
	...
	gluPerspective( 45, ( double )m_screenWidth/( double )m_screenHeight, 10.0f, 5000.0f );
...
bool DemoApp::OnRelease( void ) {
	m_ROAM.ReleaseDataSet( );
	m_ROAM.Shutdown( );

	m_shadingTexture.Release( );
