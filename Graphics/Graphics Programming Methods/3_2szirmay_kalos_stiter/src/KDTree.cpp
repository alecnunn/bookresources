...
#pragma warning( disable : 4311 )		// 'type cast' : pointer truncation from 'unsigned int *' to 'unsigned int'
#pragma warning( disable : 4312 )		// 'type cast' : conversion from 'unsigned int' to 'float *' of greater size

bool KDTree::traverse (const Ray& ray, HitRec& hitRec, float minT, float maxT)
{
	// handle dirs parallel to axis here in advance
	// will not need to check later for every node;
	Vector dirInverts;
	for(int i=0; i<3; i++) {
		dirInverts[i] = 1.0f / ray.dir[i];
	}
	...
	for(;;)
	{
			...
			if(leafList) // leaf contains some objects
			{
				...
				for (Intersectable** objectIterator = leafList; objectIterator < leafListEnd; objectIterator++) {
					...
					if (ray.id != (*objectIterator)->lastTestedRayId) 
						(*objectIterator)->intersect (ray, depth);
					else
					...
					if ((*objectIterator)->lastTestedRayResult.isIntersect && 
							depth < hitRec.depth &&
							depth > rayMin - epsilon &&
							depth < rayMax + epsilon
							)
		...
		{
			// gather cutting plane information from the nodeTable
			float nodeValue = nodeTable[tNode];
			unsigned char axis = *(unsigned int*)(&nodeValue) & 0x00000003;
			unsigned int bitfield = *(unsigned int*)(&nodeValue) & 0xfffffffc;
			float cutPlane = *(float*)(&bitfield);

			float origCoord = ray.origin[axis];
			float dirCoord = dirInverts[axis];

            // calculate cutting plane - ray intersection distance 't'
			register float t = (cutPlane - origCoord) * dirCoord;

			// find the child volume nearer to the origin
			unsigned int nearNode = rightChild;
			unsigned int farNode = leftChild;
			if(origCoord + epsilon < cutPlane ||		// origin on the left
				(cutPlane + epsilon > origCoord) &&		// or on the plane
...
float KDTree::getBoundValue(unsigned int * extremeIndex, unsigned char axis)
{
	...
	return (index & 0x80000000)?
		(objects[ index & 0x7fffffff]->bbox.maxPoint[axis]):
		(objects[ index & 0x7fffffff]->bbox.minPoint[axis]);
}
...
void KDTree::build(unsigned int nodeID, unsigned int* boundsArray, unsigned int nObjects, BoundingBox& limits, unsigned char axisNmask)
{
	...
	float leftCoesCut = objects[boundsArray[0] & 0x7fffffff]->bbox.minPoint[axis];
	float rightCoesCut = objects[boundsArray[(nObjects << 1)-1] & 0x7fffffff]->bbox.maxPoint[axis];
	...
	unsigned int minimumIntersected = 0;
	int minimumLeft = 0;
	...
	*(unsigned int*)&nodeTable[nodeID | nCacheLineNodes] &= ~(0x1 << (nodeID & nCacheLineNodes));
	...
	currentDepth++; if(currentDepth > depth) depth = currentDepth;

	// allocate child nodes, pass the objects on to them
	unsigned int leftChildMade;
	unsigned int rightChildMade;

	// handle the two branches, the one with less objects first
	bool firstBranch = false;
	bool rightBranchHasMoreObjects = (bool)((minimumLeft << 1) + minimumIntersected < nObjects);
...
KDTree::KDTree (Intersectable** iObjects, unsigned int nObjects){
	// take parameters
	...
	// determine error because of 21 bits mantissa representaion
	float largest = 0.0f;
	for(int i=0; i<3; i++)
		if( boundingBox.maxPoint[i] > largest)
			largest = boundingBox.maxPoint[i];
	for(int o=0; o<3; o++)
		if( - boundingBox.minPoint[o] > largest)
			largest = - boundingBox.minPoint[o];
	epsilon = largest / 524288.0f; // e = X * 2^(-19);
	...
	maxNCacheLines = nObjects * 16 / nCacheLineNodes + 1;
	// cache lines already allocated
	nCacheLines = 1;

	unsigned int maxPossibleFree = maxNCacheLines * nCacheLineNodes / 2;
	// allocate free nodes heap
	freeNodes = new	Heap<unsigned int>(maxPossibleFree);
	...
	traverseStack = new TraverseStack[depth];
...
bool KDTree::compare(unsigned int aIndex, unsigned int bIndex, unsigned char axis)
{
	...
	float aBegin = objects[caIndex]->bbox.minPoint[axis];
	float aEnd = objects[caIndex]->bbox.maxPoint[axis];
	unsigned int cbIndex = bIndex & 0x7fffffff;
	float bBegin = objects[cbIndex]->bbox.minPoint[axis];
	float bEnd = objects[cbIndex]->bbox.maxPoint[axis];
	...
	if(aIndex & 0x80000000)
		if(bIndex & 0x80000000)
		{
			...
			if(aEnd + epsilon < bEnd)
				return true;
			if(bEnd + epsilon < aEnd)
				return false;
			if(aBegin + epsilon < bBegin)
				return true;
			if(bBegin + epsilon < aBegin)
				return false;
		...
		else
		{
			if(caIndex == cbIndex)
				return false;
			if(aEnd + epsilon < bBegin)
				return true;
			if(bBegin + epsilon < aEnd)
				return false;
			if(aBegin + epsilon < bEnd)
				return true;
			return caIndex < cbIndex;
		...
		if(bIndex & 0x80000000)
		{
			if(caIndex == cbIndex)
				return true;
			if(bEnd + epsilon < aBegin)
				return false;
			if(aBegin + epsilon < bEnd)
				return true;
			if(bBegin + epsilon < aEnd)
				return false;
			return caIndex + epsilon < cbIndex;
		}
		else
		{
			if(caIndex == cbIndex)
...
inline unsigned int KDTree::makePointer(unsigned int originalNode)
{
	...
	else														// find free node
	{
		...
		else
		{
			...
			{
				maxNCacheLines = (unsigned int)(maxNCacheLines * 1.1f);
