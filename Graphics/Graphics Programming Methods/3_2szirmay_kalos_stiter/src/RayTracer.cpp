...
// a minimal ray tracing framework for demonstration

#include "raytracer.h"
#include "stdio.h"

#ifdef NEVER
// set up a simple scene for demonstration
			...
			for(int depth=0; depth < maxSteps; depth++)
			{
				...
				for(int iLight=0; iLight < nVirtualWorldLights; iLight++)
				{
					...
					if(!shadowHit.isIntersect)
					{
						...
						hitRec.object->material->ScatteringProbability(ray.dir, shadowRay.dir, normal, lightBrdfValue);
						radianceInPixel += (idealBrdf % lightBrdfValue % virtualWorldLights[iLight].radiance) * (1.0f / (distance * distance));
					}
				}
...
// save rendered image as a TGA file
