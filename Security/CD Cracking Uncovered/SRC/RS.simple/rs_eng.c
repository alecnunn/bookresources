...
// specify irreducible polynomial coefficients (see ECMA-130)
...
generate_gf()
{
	...
	for (i = 0; i < mm; i++)
	{
		...
		mask <<= 1;
	} index_of[alpha_to[mm]] = mm;  mask >>= 1;
...
// nn=(2**mm - 1) ReedSolomon code from the product of (X+alpha**i), i=1..2*tt
//----------------------------------------------------------------------------
gen_poly()
{
	int i, j;
	...
	for (i=2; i<=nn-kk; i++)
	{
		...
		for (j = i-1; j > 0; j--)
		{
			if (gg[j]) gg[j]=gg[j-1]^alpha_to[(index_of[gg[j]]+i)%nn];
				else
			gg[j]=gg[j-1];
		}
...
void decode_rs()
{
	...
	{
		s[i] = 0 ;
		for (j = 0; j < nn; j++)	/* recd[j] in index form */
			if (recd[j] != -1) s[i] ^= alpha_to[(recd[j]+i*j)%nn];
	...
	if (syn_error)					// if errors, try and correct
	{
		...
		step number  ranging  from -1 to 2*tt (see L&C), l[u] is the degree of
		the elp at that step, and u_l[u] is the  difference  between  the step
		...
		if (l[u]<=tt)		// can correct error
		{
			...
			for (i = 1; i <= nn; i++)
			{
				...
				for (j = 1; j <=l [u]; j++)
				{
					if (reg[j]!=-1)
					{
						reg[j] = (reg[j]+j)%nn; q ^= alpha_to[reg[j]];
					}
				}
			...
			for (i=0; i<l[u]; i++) // compute numerator of error term first
			{
				...
				if (err[loc[i]]!=0)
				{
					...
					q = q % nn; err[loc[i]] = alpha_to[(err[loc[i]]-q+nn)%nn];
...
main()
{
	...
	// for known data, stick a few numbers into a zero codeword.
	// Data is in polynomial form.
