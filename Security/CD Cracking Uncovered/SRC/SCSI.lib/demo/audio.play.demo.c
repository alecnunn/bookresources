...
//		----------------
//
// CD-PLAYER kernel contains functions that directly control the drive - 
// _CD_READ_TOC, _GET_CURRENT and _PLAY_CD_MSF



//=[_krnl_READ_TOC(char *CD, unsigned char *_TOC)=============================
//
//		Read TOC in 00h format (i.e., only AAh as Lead-Out)
//
//	ARG:
//		CD			- drive name/address
//		_TOC		- pointer to buffer[MAX_TOC_SIZE] fro copying TOC
//
//	RET:
//		 SCSI_OK	- success
//		~SCSI_OK	- error
//
//	NOTE:
//		The function returns tracks of all sessions, including all data tracks, without
// trying to sort them, because this is the application-level task
// 
_krnl_READ_TOC(char *CD, unsigned char *_TOC)
{
	int a;
	...
	// sending the command to read the TOC
	return SCSI_SEND_CMD(cd, CDB, ATAPI_CDB_SIZE, NO_SENSE, _TOC, MAX_TOC_SIZE, SCSI_DATA_IN);
}


//=[_krnl_GET_CURRENT(char *cd, ULBA *lba, ULBA *track_lba, int *track)]======
//
//		
_krnl_GET_CURRENT(char *cd, ULBA *lba, ULBA *track_lba, int *track)
{
...
FACK_TOC(char *cd, int TRACK_LBA, int DO)
{
	...
	for (a = 4; a < len; a += sizeof(struct CD_TOC))
	{
		...
		// track++; No, tracks may have irregular numbering
	}
...
// returns the location until which to playback
