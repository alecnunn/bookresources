...
main(int argc, char** argv)
{
	...
	for (x = LBA; x <= argLBAmax; x++)
	{
		if ((argLBAmax - LBA) > 25)
			fprintf(stderr, "\rdo %d%%\r", ((x - LBA)*100)/(argLBAmax - LBA));
		...
		a = SEND_SCSI_CMD(CD, CDB, ATAPI_CDB_SIZE, SENSE, buf, MAX_BUF_SIZE, SCSI_DATA_IN);
		if (a!=SCSI_OK)
		{
			fprintf(stderr,"-ERR:error seek\n"); 
			printf("SENSE:"); for (a = 0; a < 20; a ++) printf("%02X ",SENSE[a]);
			printf("\n"); 
